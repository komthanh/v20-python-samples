...
def main():
    ...
    args = parser.parse_args("EUR_USD 1 1.167".split())
    api = args.config.create_context()
    orderArgs.parse_arguments(args)

    if args.replace_order_id is not None:
        response=api.order.stop_replace(args.config.active_account,args.replace_order_id,**orderArgs.parsed_args)
    else:
        response=api.order.stop(args.config.active_account,**orderArgs.parsed_args)
