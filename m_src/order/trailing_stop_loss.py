...
def main():
    ...
    args = parser.parse_args("52 0.1 --tif GTC".split())
    ...
    if args.replace_order_id is not None:
        response = api.order.trailing_stop_loss_replace(args.config.active_account, args.replace_order_id, **orderArgs.parsed_args)
    else:
        response = api.order.trailing_stop_loss(args.config.active_account, **orderArgs.parsed_args)
