...
def main():
    ...
    args = parser.parse_args("EUR_USD 1 1.164".split())
    api = args.config.create_context()
    orderArgs.parse_arguments(args)

    if args.replace_order_id is not None:
        response=api.order.market_if_touched_replace(args.config.active_account,args.replace_order_id,**orderArgs.parsed_args)
    else:
        response=api.order.market_if_touched(args.config.active_account,**orderArgs.parsed_args)
